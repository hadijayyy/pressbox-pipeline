...
if [ $EXIT_CODE -eq 0 ] && [ -f /tmp/pressbox-last-report ]; then
    ...
    echo "$MSG"
    notify "$MSG"
    # Send last 10 lines of log for diagnosis
    LOG_TAIL=$(tail -10 /tmp/pressbox-mvp.log 2>/dev/null)
    notify "Log tail: $LOG_TAIL"
fi
