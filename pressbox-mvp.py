...
def main():
    ...
    if not article_accepted or not slides:
        # Fallback: generate minimal simple slides so pipeline does not abort
        log("⚠️ All candidates rejected – generating fallback content")
        # Use best article title as hook, even if not fully vetted
        fallback_title = best.get('title', 'No title')
        fallback_source = best.get('source', 'unknown')
        fallback_slides = [
            {"title": "S1", "content": f"{fallback_title} – {fallback_source}. Why should you care?"},
            {"title": "S2", "content": "No concrete facts yet. Stay tuned for updates."},
            {"title": "S3", "content": "Context missing – story may develop later."},
            {"title": "S4", "content": "Impact unclear – watch official statements."},
            {"title": "S5", "content": "No clear take, but keep an eye on this."},
            {"title": "S6", "content": "Agree or disagree – does this matter to you?"},
        ]
        caption = f"{fallback_title} – {fallback_source}"
        hashtags = "#football #news"
        # Prepare dict compatible with later code expectations
        slides = fallback_slides
        # Attach extra fields for posting/logging later
        best["caption"] = caption
        best["hashtags"] = hashtags
        # Continue as if generation succeeded (skip exit)
        article_accepted = True
        
        # Log fallback creation
        log("✅ Fallback slides created")
        # No sys.exit – continue pipeline
        
        # (If DRY_RUN later will print these slides)
        
        # Note: `best` already contains url/source info for posting
        
        # End of fallback block
        
        # (original exit removed)
...
# End of fallback block – no stray quote
